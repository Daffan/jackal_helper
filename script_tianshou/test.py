...
if not model_path.startswith('../'):
    model_path = '../results/' + model_path + '/policy_30.pth'
else:
    model_path = model_path + '/policy_30.pth'
...
model = Net(training_config['layer_num'], state_shape, action_shape, config['device']).to(config['device'])
...
for i in range(avg):
    ...
    while not done:
        actions = np.array(model(torch.tensor([obs]))[0].detach().cpu())
